            ...
            <div class="new-51">
                ...
                <div class="content">
                    ...
                    <div class="list list-padding-without-bottom">
                        ...
                        <div class="row-game inline"  v-for="(item, index) in newGames" :key="index" style="border-bottom: 0.02rem solid #F5F5F5;">
                            ...
                            <div class="game-info">
                                ...
                                <div class="des">{{item.create_time}}</div>

                            </div>
                            <div class="btn-border">
                                <div class="name row-name-price">
                                    {{item.amount}} {{item.coin_name}}
                                </div>
                                <div class="des">{{item.status_name}}</div>
                            </div>
                        </div>
            ...
            loadRemote() {
                this.getMGCGameCenterData().then(mgcResp => {
                    if(mgcResp && mgcResp.data && mgcResp.data.code == 200 && mgcResp.data.data) {
                        ...
                        // get banner data
                        let dataList = mgcResp.data.data || []
                        for(let idx in dataList) {
                            let data = dataList[idx]
                            let type = data['type']
                            let status = data['status']
                            if(1 == type){
                                dataList[idx]['content_name'] = '提现到微信零钱'
                            }else if (2 == type){
                                dataList[idx]['content_name'] = '支付宝提现'
                            }else if (3 == type){
                                dataList[idx]['content_name'] = '提现到银行卡'
                            }else if (4 == type){
                                dataList[idx]['content_name'] = '步数宝提现'
                            }else if (5 == type){
                                dataList[idx]['content_name'] = '第三方兑换'
                            }else{
                                dataList[idx]['content_name'] = '提现到第三方微信零钱'
                            }

                            if(1 == status){
                                dataList[idx]['status_name'] = '等待审核'
                            }else if (2 == status){
                                dataList[idx]['status_name'] = '预到账成功'
                            }else if (3 == status){
                                dataList[idx]['statust_name'] = '审核失败'
                            }else if (4 == status){
                                dataList[idx]['status_name'] = '提现失败'
                            }else if (5 == status){
                                dataList[idx]['status_name'] = '到账成功'
                            }else{
                                dataList[idx]['status_name'] = '到账失败'
                            }
                        }
                        // return
                        if( 0 == Object.keys(this.json_data).length){
                            this.json_data = dataList;
                        }else {
                            for (var p in dataList){
                                var game_id = dataList[p]['id'];
                                var is_push  = 1;
                                for (var s in this.json_data){
                                    var game_id_exit = this.json_data[s]['id'];
                                    if(game_id_exit == game_id){
                                        is_push = 2;
                                    }
                                }
                                if(1 == is_push){
                                    this.json_data.push(dataList[p]);
                                }
                            }
                        }
                        this.newGames = this.json_data;
                        const num = 10;//每一页接受多少条数据
                        if(dataList.length<num){
                            this.footerText = "— 没有更多内容 —";
